...
abstract class Scrypt
{
    ...
    {
        if ($n == 0 || ($n & ($n - 1)) != 0) {
            return \myApp\Error::onError("N must be > 0 and a power of 2");
        }
        if ($n > PHP_INT_MAX / 128 / $r) {
             return \myApp\Error::onError("Parameter n is too large");
        }
        if ($r > PHP_INT_MAX / 128 / $p) {
             return \myApp\Error::onError("Parameter r is too large");
        }

        if (extension_loaded('Scrypt')) {
            if ($length < 16) {
                 return \myApp\Error::onError("Key length is too low, must be greater or equal to 16");
            }
